    ...
    if (mShouldCheckLocInWindow && parentY == 0) {
      ...
      try {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.HONEYCOMB) {
          y = activity.getWindow().getDecorView().findViewById(android.R.id.content).getY();
        }
        //y==0的时候，就代表content的布局直接顶到了屏幕顶部，就需要添加padding了。
        if (y == 0) {
          parentY = activity.getResources().getDimensionPixelSize(i5)  - Common.dip2px(activity, 25);
        } else {
